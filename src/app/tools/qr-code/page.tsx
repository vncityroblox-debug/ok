...
function buildQrData(
  ...
  switch (tab) {
    ...
    case 'wifi':
      return `WIFI:T:${inputs.encryption};S:${inputs.ssid};P:${inputs.password};;`;
    default:
      return '';
  }
}
...
export default function QRCodePage() {
  ...
  const handleCopyLink = useCallback(async () => {
    try {
      ...
      window.prompt('Copy this link:', apiUrl);
    }
  }, [apiUrl]);
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px 60px' }}>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 20 }}>
              ...
              {activeTab === 'wifi' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div>
                    <label style={labelStyle}>SSID (Network Name)</label>
                    <input
                      type="text"
                      placeholder="Tên mạng WiFi"
                      value={inputs.ssid}
                      onChange={(e) => updateInput('ssid', e.target.value)}
                      style={inputStyle}
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select
                      value={inputs.encryption}
                      onChange={(e) => updateInput('encryption', e.target.value)}
          ...
          >
            ...
            <div
              style={{
                width: Math.max(...sizeOptions.map((s) => Number(s.split('x')[0]))),
                maxWidth: '100%',
                aspectRatio: '1',
                display: 'flex',
                alignItems: 'center',
