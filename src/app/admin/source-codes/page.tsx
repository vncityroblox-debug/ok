...
export default function AdminSourceCodes() {
  ...
  async function fetchData() {
    ...
    try {
      const [catsRes, appsRes, keysRes] = await Promise.all([
        ...
        supabase.from('apps').select('*, categories(name)').eq('app_type', 'source_code').order('created_at', { ascending: false }),
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('apps').insert({ name: name.trim(), slug, description: description.trim(), category_id: categoryId || null, main_image_url: mainImageUrl.trim(), detail_images: detailImages, download_link: downloadLink.trim(), is_locked: isLocked, app_type: 'source_code' });
      ...
      fetchData();
    } catch (err: any) { setErrorMsg(err.message || 'Lỗi khi đăng mã nguồn.'); } finally { setIsSubmitting(false); }
  };
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      ...
      if (error) throw error;
      setSuccessMsg('Cập nhật mã nguồn thành công!'); setEditingId(null); fetchData();
    } catch (err: any) { setErrorMsg(err.message || 'Lỗi khi cập nhật mã nguồn.'); }
  };

  const handleDelete = async (id: string, appName: string) => {
    if (confirm(`Bạn có chắc chắn muốn xóa mã nguồn "${appName}"?`)) {
      setErrorMsg(''); setSuccessMsg('');
      try {
        const { error } = await supabase.from('apps').delete().eq('id', id);
        if (error) throw error;
        setSuccessMsg(`Đã xóa mã nguồn "${appName}".`); fetchData();
      } catch (err: any) { setErrorMsg(err.message || 'Lỗi khi xóa mã nguồn.'); }
    }
  };
  ...
  const handleBulkAction = async () => {
    ...
    try {
      if (bulkAction === 'lock') {
        ...
        if (error) throw error;
        setSuccessMsg(`Đã gắn Key cho ${ids.length} mã nguồn.`);
      } else if (bulkAction === 'detach_key') {
        const { error } = await supabase.from('app_keys').delete().in('app_id', ids);
        if (error) throw error;
        setSuccessMsg(`Đã tháo Key khỏi ${ids.length} mã nguồn.`);
      }
  ...
  return (
    <div>
      <div className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>Quản Lý Mã Nguồn</h1>
        <p className={styles.pageSubtitle}>Đăng tải mã nguồn mới, thiết lập trạng thái Khóa/Mở, và cấu hình thông tin tải về.</p>
      </div>
      ...
      {editingId === null && (
        <form onSubmit={handleCreate} className="glass-panel" style={{ padding: '30px', marginBottom: '40px' }}>
          <h3 style={{ marginBottom: '24px', fontWeight: 700, fontSize: '1.25rem' }}>Đăng Mã Nguồn Mới</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px' }}>
            <div className={styles.formGroup}>
              <label className={styles.formLabel} htmlFor="appName">Tên Mã Nguồn</label>
              <input id="appName" type="text" required className={styles.formInput} value={name} onChange={(e) => setName(e.target.value)} placeholder="Nhập tên mã nguồn" />
            </div>
            <div className={styles.formGroup}>
          ...
          <div className={styles.formGroup}>
            <label className={styles.formLabel} htmlFor="appDesc">Mô Tả Mã Nguồn</label>
            <textarea id="appDesc" rows={4} required className={styles.formInput} style={{ resize: 'vertical' }} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Mô tả chi tiết tính năng của mã nguồn..." />
          </div>
          ...
          <button type="submit" className="neon-btn" style={{ marginTop: '10px' }} disabled={isSubmitting}>
            <AppWindow size={18} />
            {isSubmitting ? 'Đang đăng tải...' : 'Đăng Mã Nguồn'}
          </button>
        </form>
      )}
      ...
      {editingId !== null && (
        <form onSubmit={(e) => { e.preventDefault(); handleUpdate(editingId); }} className="glass-panel" style={{ padding: '30px', marginBottom: '40px', border: '1px solid hsl(var(--color-primary))' }}>
          <h3 style={{ marginBottom: '24px', fontWeight: 700, fontSize: '1.25rem', color: 'hsl(var(--color-primary))' }}>Chỉnh Sửa Mã Nguồn</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px' }}>
            <div className={styles.formGroup}>
              <label className={styles.formLabel}>Tên Mã Nguồn</label>
          ...
          <div className={styles.formGroup}>
            <label className={styles.formLabel}>Mô Tả Mã Nguồn</label>
      ...
      <div className="glass-panel" style={{ padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h3 style={{ fontWeight: 700, fontSize: '1.25rem' }}>Kho Mã Nguồn</h3>
          {selectedIds.size > 0 && (
        ...
        {isLoading ? (
          <p style={{ color: 'hsl(var(--text-secondary))', textAlign: 'center', padding: '40px 0' }}>Đang tải dữ liệu mã nguồn...</p>
        ) : apps.length === 0 ? (
          <p style={{ color: 'hsl(var(--text-muted))', textAlign: 'center', padding: '40px 0' }}>Chưa có mã nguồn nào được đăng.</p>
        ) : (
