...
export default function AdminSourceCodes() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Lỗi khi đăng mã nguồn.');
    } finally {
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Lỗi khi cập nhật mã nguồn.');
    }
  };
  ...
  const handleDelete = async (id: string, appName: string) => {
    if (confirm(`Bạn có chắc chắn muốn xóa mã nguồn "${appName}"?`)) {
      ...
      try {
        ...
        setErrorMsg(err.message || 'Lỗi khi xóa mã nguồn.');
      }
    }
  };
    ...
    <div>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageSubtitle}>
          Đăng tải mã nguồn mới, thiết lập trạng thái Khóa/Mở, và cấu hình thông tin tải về.
        </p>
      </div>
        ...
        <form onSubmit={handleCreate} className="glass-panel" style={{ padding: '30px', marginBottom: '40px' }}>
          ...
          <div className={styles.formGroup}>
            ...
            <textarea
              ...
              placeholder="Mô tả chi tiết tính năng của mã nguồn..."
            />
          </div>
